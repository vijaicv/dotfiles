-- nvim-jdtls: Java LSP via eclipse.jdt.ls.
-- Install jdtls with :MasonInstall jdtls (or have jdtls in PATH).
-- See :help jdtls and https://github.com/mfussenegger/nvim-jdtls

local function get_mason_package_path(name)
  -- Mason API: get_install_path() was removed in newer mason.nvim; use install root.
  local ok, settings = pcall(require, "mason.settings")
  if ok and settings and settings.current then
    return settings.current.install_root_dir .. "/packages/" .. name
  end
...
local function get_jdtls()
  ...
  -- path to jar that run language server
  local launcher = vim.fn.glob(jdtls_path .. "/plugins/org.eclipse.equinox.launcher_*.jar")
  local SYSTEM = "linux"
  local config = vim.fn.glob(jdtls_path .. "/config_" .. SYSTEM)
...
local function get_bundles()
  ...
  local jars = vim.split(vim.fn.glob(java_test_path .. "/extension/server/*.jar"), "\n")
...
local function get_workspace()
    ...
    local workspace_path = home .. "/projects/projects-meta/"
...
local function java_keymaps()
  ...
  map("n", "<A-o>", "<Cmd>lua require('jdtls').organize_imports()<CR>", { buffer = true, desc = "Organize imports" })
  map("n", "crv", "<Cmd>lua require('jdtls').extract_variable()<CR>", { buffer = true, desc = "Extract variable" })
  map("v", "crv", "<Esc><Cmd>lua require('jdtls').extract_variable(true)<CR>", { buffer = true, desc = "Extract variable (visual)" })
  map("n", "crc", "<Cmd>lua require('jdtls').extract_constant()<CR>", { buffer = true, desc = "Extract constant" })
  map("v", "crc", "<Esc><Cmd>lua require('jdtls').extract_constant(true)<CR>", { buffer = true, desc = "Extract constant (visual)" })
  map("v", "crm", "<Esc><Cmd>lua require('jdtls').extract_method(true)<CR>", { buffer = true, desc = "Extract method" })
end
...
local function setup_jdtls()
    ...
    local root_dir = jdtls.setup.find_root({ '.git',  'gradlew', 'mvnw', 'pom.xml', 'build.gradle' });
